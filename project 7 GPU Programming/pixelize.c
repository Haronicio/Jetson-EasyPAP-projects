...
// ./run -k pixelize -l images/1024.png -n --gpu --variant ocl -i 1
...
unsigned pixelize_invoke_ocl (unsigned nb_iter)
{
  ...
  ocl_retrieve_data(); 
  graphics_dump_image_to_file("result_gpu_pixelize.png");
...
// ./run -k pixelize -l images/1024.png -n --gpu --variant ocl_red -i 1 -tw 16 -th 16

unsigned pixelize_invoke_ocl_red (unsigned nb_iter)
{
  size_t global[2] = {GPU_SIZE_X, GPU_SIZE_Y};
  size_t local[2]  = {TILE_W, TILE_H};
  cl_int err;
...
void pixelize_config_ocl_red (char *param)
{
  pixelize_config (param);
...
void pixelize_init_ocl_red (void)
{
  if (TILE_W != TILE_H)
    exit_with_error ("Tiles should have a square shape (%d != %d)", TILE_W,
                     TILE_H);
